...
; 三个参数－文件大小　源地址　目的地址已经在调用之前分别压栈保存
; ------------------------------------------------------------------------
MemCpy:
	push	ebp
	mov	ebp, esp
...
InitKernel:
        xor   esi, esi
        mov   cx, word [BaseOfKernelFilePhyAddr+2Ch]; BaseOfKernelFilePhyAddr代表了kernel.bin的首地址
                                                    ; +2Ch正好是程序头数目，表示该ELF文件中有多少个程序头
        movzx ecx, cx                               ; 无符号整数0位扩展传送
        mov   esi, [BaseOfKernelFilePhyAddr + 1Ch]  ; +1Ch正好是程序头表想对于kernel.bin文件的偏移地址
        add   esi, BaseOfKernelFilePhyAddr          ; esi保存了程序头表在内存中位置
.Begin:
        mov   eax, [esi + 0]              ; 程序头表前四个字节表示该段的类型  
        cmp   eax, 0                      ; 判断是不是无效段-PT_NULL
        ...
        jz    .NoAction
        push  dword [esi + 010h]    ;size ; +010h，表示段大小，作为MemCpy的第三个参数入栈
        mov   eax, [esi + 04h]            ; +04h，表示段的偏移
        add   eax, BaseOfKernelFilePhyAddr;　加上文件的基址得到段的源地址，　作为MemCpy的第二个参数入栈
        push  eax		    ;src  ; |      uchCode + pPHdr->p_offset,
        push  dword [esi + 08h]     ;dst  ; +08h，表示段在内存中的虚拟地址，　得到目的地址，　作为MemCpy的第一个参数入栈
        call  MemCpy                      ; |
        add   esp, 12                     ;　将之前保存的参数弹栈不保存，回复堆栈
.NoAction:
        add   esi, 020h                   ; 读取下一个程序头表，一个程序头表大小为32个字节，02h
        dec   ecx                         ; 程序头表数目减１，用于循环
        jnz   .Begin
