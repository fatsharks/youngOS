...
LABEL_FILE_LOADED:
    ...
    ; 线性地址等于物理地址
    ; 没有在16位代码段初始化选择子
    ; 另外这里为什么的偏移地址为什么是BaseOfLoaderPhyAddr+LABEL_PM_START，而为什么不是直接就是LABEL_PM_START,这是因为我们的代码段设置的段基址位0，那么当我们拿段选择字索引到该代码段后，于是从段寄存器的高速缓存处获得了段基地址，而此时我们的Loader.bin是被加载到段地址为0900，于是我们这里需要加上BaseOfLoaderPhyAddr，BaseOfLoaderPhyAddr此时我们设置为09000h，那么就相当于我们在实模式进行段地址*16+偏移地址的思想差不多
