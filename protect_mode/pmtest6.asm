...
SetupPaging:
	...
	xor	edi, edi
	xor	eax, eax
	mov	eax, PageTblBase | PG_P  | PG_USU | PG_RWW;赋值指向页表地址，所以首地址为页表首地址
.1:
	stosd				; stosd-字符串操作指令，EAX的内容存入由EDI中偏移量指向的内存位置,EDI根据方向标志位的状态递增或递减
	...
	xor	edi, edi
	xor	eax, eax
	mov	eax, PG_P  | PG_USU | PG_RWW			;赋值为物理页地址，所以首地址从零开始
.2:
	stosd
